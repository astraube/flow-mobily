/**
 * by https://github.com/DesignRevision/shards-vue
 */
...
export function vueUse(VuePlugin, definition) {
  if (typeof window !== 'undefined' && window.Vue) {
    window.Vue.use(VuePlugin, definition)
  }
}
...
export const arrayFilter = function(arr, key='toMenu', value='drawer') {
  if (!isArray(arr)) {
    return arr;
  }
 ...
 * Json methods
 */
export const jsonFlatten = function(obj, keyChild='children', root='') {
  let funcName = new Error().stack.match(/at (.*?) /)[1];
  let func = eval(funcName);

  if (isArray(obj)) {
    obj['root'] = root
    return obj.map(x => func(x, keyChild, root)).flat(Infinity);

  } else if (obj[keyChild]) {
    obj['root'] = root
    return func(obj[keyChild], keyChild, obj['path']);
  }
  obj['root'] = root
  return obj;
}

export const jsonPretty = function (json) {
  if (json) {
