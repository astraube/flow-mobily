<template>
  <div class="q-pb-md user-settings full-width" v-if="currentUser">
    <q-form class="full-height" @submit="saveUserData">
      ...
      <div class="absolute-top q-mt-sm text-white text-center">
        <q-icon class="q-mr-sm" color="white" name="edit" size="20px" @click="showPhotoUpload('profile')"></q-icon>Editar seu plano de fundo</div>
      <div class="profile-photo text-center" @click="showPhotoUpload('profile')">
        <div class="default-user-image column items-center" v-if="showDefaultPhoto()">
          <q-avatar class="q-mb-sm" round="round" color="blue-grey-10" icon="person" font-size="110px" size="180px" text-color="white"></q-avatar><span class="text-caption text-blue-grey-10">Click para editar</span></div>
        <div class="user-image column items-center" v-else>
          ...
          <span class="text-blue-grey-10">
            <q-icon class="q-mr-sm" color="blue-grey-10" name="edit" size="16px"></q-icon>Click para editar
          </span>
        </div>
      </div>
      <section class="user-info">
        <h6 class="q-mt-none q-mb-md text-center">Edite seu perfil</h6>
        <div class="row justify-between items-center q-mb-lg"><label class="col-3" for="displayName">Nome</label>
          <q-input class="col" id="displayName" v-model="displayName" borderless="borderless" dense="dense" type="text"></q-input>
        </div>
        <div class="row justify-between items-center q-mb-lg"><label class="col-3" for="email">E-mail</label>
          <q-input class="col" id="email" v-model="email" borderless="borderless" dense="dense" type="text"></q-input>
        </div>
        <div class="row justify-between items-center q-mb-lg"><label class="col-3" for="phone">Telefone   celular</label>
          <q-input class="col" id="phone" v-model="phone" borderless="borderless" dense="dense" hint="(##) #####-####" mask="(##) #####-####" type="text"></q-input>
        </div>
      </section>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async saveUserData () {
        const { currentUser, email, displayName, phone } = this
